...
const sendPaymentResult = (data) => {
  ...
  const transporter = nodemailer.createTransport({
    ...
    auth: {
      ...
      clientSecret: config.emailClientSerect,
      refreshToken: config.emailRefleshToken
    }
  });
  // 信件內容
  const mailOptions = {
    from: `Quapni-康迪薾戶外 <${config.emailSender}>`,
    to: data.CustomField2,
    cc: config.emailSender,
    subject: 'Quapni線上購物-訂單付款成功通知',
...
如有任何問題，也歡迎使用客服信箱聯絡我們，我們將竭誠為您服務。<br/>
客服信箱： <a href="mailto:service@quapni.com.tw">service@quapni.com.tw</a><br/>
聯絡電話： <a href="tel:0425605778">04-25605778</a><br/>
官網: <a href="https://quapni.com/">https://quapni.com/</a><br/>
FB粉絲團: <a href="https://www.facebook.com/quapni">https://www.facebook.com/quapni</a><br/><br/>
<img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTOXCp_29ppz0xOPP-ZrMhgaETJkt1mF2HJyXzh4AmJ-SQqTc0qNg" width="100">
-康迪薾戶外 小組敬上<br/><br/>
...
const sendOrderResult = (orderInfo, data) => {
  ...
  const mailOptions = {
    from: `Quapni-康迪薾戶外 <${config.emailSender}>`,
    to: data.CustomField2,
    subject: `Quapni線上購物-購買清單(訂單編號: ${data.MerchantTradeNo})`,
    html: `親愛的 顧客 您好：<br/>

已收到您的訂購資訊，感謝您訂購 Quapni 產品！本通知函只是通知您本系統已收到您的訂購訊息、並供您再次自行核對之用，不代表交易已完成。<br/>
<span style="color:red;">[防詐騙提醒]</span> 若您接獲任何電話要您依照指示操作ATM，提供剩額、變更付款方式或更改分期設定等，請不要依電話指示操作，建議您直接與本公司客服中心聯繫確認，謝謝您！<br/><br/><br/>
<!-- table表單 -->
${TABLE}
...
const queryTradeInfo = (merchantTradeNo) => {
  return new Promise((resolve, reject) => {
    ...
    // const originString = `HashKey=ww4icIu3RuqDAc74&MerchantID=3043395&MerchantTradeNo=${merchantTradeNo}&TimeStamp=${timestamp}&HashIV=GDiTbnPGuFtsuhqQ`;
    const originString = `HashKey=${config.ECPayHashKey}&MerchantID=${config.ECPayMerchantID}&MerchantTradeNo=${merchantTradeNo}&TimeStamp=${timestamp}&HashIV=${config.ECPayHashIV}`;
    ...
    request.post({ url: config.ECPayTradeQuery, form: formData }, (err, httpResponse, body) => {
      // 解析訂單資料(字串轉JSON)
